...
def crawl_content_of_list_post():
    for link_post in LIST_POST:        
        time.sleep(1)          
        data_to_json=get_content_post(link_post) # lay du lieu bai viet
        print(convert_json_create_file(data_to_json, get_file_name(link_post))) # tao file
...
def get_file_name(link_target):
    ...
    name_file_json = process_name_file_json[6].split(".")
